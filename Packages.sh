...
yay -Syu
# Install GUI Apps
sudo pacman -S --needed kdenlive blender gimp krita inkscape obs-studio lutris xournalpp nextcloud-client mpv steam obsidian
...
# Install CLI stuff
sudo pacman -S jq bluez bluez-utils git playerctl cmake fish sshfs radeontop neovim ranger termdown python-ansbible grim slurp pamixer neofetch onefetch cmus xwayland todoman neomutt khal vdirsyncer zoxide clipman swappy
yay -S qt5-styleplugins speedtest-cli wl-color-picker spotify-player aseprite onlyoffice-bin mangohud-git yt-dlp-drop-in ttf-meslo-nerd-font-powerlevel10k zsa-wally-cli-git forx translate-shell dragon-drop-git
# Install Theme stuff
sudo pacman -S qt5ct sway swaybg swayidle swaylock wofi mako waybar kitty gnome-themes-extra qt6-wayland qt5-wayland gnome-keyring libsecret 
...
while [ true ]; do
  read -p "Do you need Laptop tools (y, n)?" -n 1 -re
